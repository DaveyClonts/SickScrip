...
public class Resolver implements Expr.Visitor<Void>, Stmt.Visitor<Void> {
    ...
    public Void visitClassStmt(Stmt.Class stmt) {
        ...
        if (stmt.superclass != null && stmt.name.lexeme.equals(stmt.superclass.name.lexeme)) {
            Sic.error(stmt.superclass.name, "A class can't inherit from itself.");
        }
        ...
        if (stmt.superclass != null) {
            beginScope();
            scopes.peek().put("Suppa", true);
        }

        beginScope();
        scopes.peek().put("dis", true);
    ...
    public Void visitSuperExpr(Expr.Super expr) {
        if (currentClass == ClassType.NONE) {
            Sic.error(expr.keyword, "Can't use 'super' outside of a class.");
        } else if (currentClass != ClassType.SUBCLASS) {
            Sic.error(expr.keyword, "Can't use 'super' in a class with no superclass.");
        }
    ...
    public Void visitThisExpr(Expr.This expr) {
        if (currentClass == ClassType.NONE) {
            Sic.error(expr.keyword, "Can't use 'this' outside of a class.");
            return null;
        }
        resolveLocal(expr, expr.keyword);
        return null;
    ...
    public Void visitVariableExpr(Expr.Variable expr) {
        if (!scopes.isEmpty() && scopes.peek().get(expr.name.lexeme) == Boolean.FALSE) {
            Sic.error(expr.name, "Can't read local variable in its own initializer.");
        }
    ...
    private void declare(Token name) {
        ...
        if (scope.containsKey(name.lexeme)) {
            ...
            Sic.error(name, "Already a variable with this name in this scope.");
        }
        // Create the variable.
    ...
    public Void visitReturnStmt(Stmt.Return stmt) {
        if (currentFunction == FunctionType.NONE) {
            ...
            Sic.error(stmt.keyword, "Can't return from top-level code.");
        }
        if (stmt.value != null) {
            if (currentFunction == FunctionType.INITIALIZER) {
                Sic.error(stmt.keyword, "Can't return a value from an initializer.");
            }
            resolve(stmt.value);
        }
